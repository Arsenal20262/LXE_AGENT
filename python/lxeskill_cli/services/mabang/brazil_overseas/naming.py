...
_FILENAME_PREFIXES = {
    ...
    BrazilExportKind.ALLOCATION_SIGNED_ALL: "马帮系统-已签收-巴西海外仓",
    BrazilExportKind.ALLOCATION_PENDING_DEFAULT_3M: "马帮系统-3个月待签收-巴西海外仓",
}


def output_filename(kind: BrazilExportKind, *, executed_at: datetime) -> str:
    """Return the approved source-workbook name in Beijing local time."""
    if executed_at.tzinfo is None:
        raise ValueError("executed_at must be timezone-aware")
    return f"{_FILENAME_PREFIXES[kind]}-{executed_at.astimezone(BEIJING_TZ):%Y-%m-%d_%H%M}.xlsx"
