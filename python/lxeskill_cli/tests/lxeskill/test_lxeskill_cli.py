...
def test_catalog_defines_every_cli_command_and_hidden_alias() -> None:
    ...
    assert sum(bool(entry.get("module")) for entry in catalog.values()) == 35
...
def test_list_and_help_write_one_terminal_jsonl_record(capsys) -> None:
    ...
    assert len(records[0]["data"]["commands"]) == 39

    assert lxeskill.main(["fba", "customs", "preview", "--help"]) == 0
    records = _records(capsys)
    assert len(records) == 1
    assert records[0]["data"]["command"] == "fba customs preview"
    ...
    assert records == [
        {
            ...
            "data": {
                "catalog_commands": 41,
                "business_commands": 35,
                "skill_files": 57,
                "owner_skills": 25,
                "command_declarations": 35,
            },
            "files": [],
        }
    ]
...
@pytest.mark.parametrize(
    ("arguments", "field"),
    [
        (["fba", "customs", "preview", "--input-xlsx", "C:/uploads/order.xlsx"], "template_xlsx"),
        (["fba", "customs", "preview", "--template-xlsx", "C:/uploads/template.xlsx"], "input_xlsx"),
...
def test_stored_asset_fills_an_omitted_slot_field_and_is_reported(
    ...
    assert lxeskill.main(["fba", "customs", "preview", "--input-xlsx", "C:/uploads/SP1-美国.xlsx"]) == 0
...
def test_supplied_asset_is_promoted_only_after_the_command_succeeds(
    ...
    assert lxeskill.main([
        "fba", "customs", "preview", "--input-xlsx", "C:/uploads/SP1-美国.xlsx",
...
def test_skill_scope_filters_list_and_blocks_out_of_scope_commands(monkeypatch, capsys) -> None:
    ...
    assert lxeskill.main(["describe", "fba", "customs", "preview"]) == lxeskill.EXIT_ENVIRONMENT
    (denied,) = _records(capsys)
    assert denied["ok"] is False
    assert denied["error"]["code"] == "skill_not_in_scope"

    assert lxeskill.main(["fba", "customs", "preview"]) == lxeskill.EXIT_ENVIRONMENT
...
def test_empty_skill_scope_denies_business_commands_but_keeps_infrastructure(monkeypatch, capsys) -> None:
    ...
    (record,) = _records(capsys)
    commands = {item["command"] for item in record["data"]["commands"]}
    assert commands == {"auth refresh", "auth browser bind", "auth browser status", "auth browser unbind"}
