...
describe("desktop agent protocol", () => {
  ...
  test("strictly parses bounded Zhihui UI progress without raw stdout", () => {
    const event = {
    ...
    for (const payload of [
      ...
      { ...event.payload, message: "智汇 TMS：token=secret" },
      { ...event.payload, message: "智汇 TMS：" + "x".repeat(121) },
