...
describe("Desktop Zhihui confirmation route", () => {
  const cliResult = (action: string, ok = true) => ({
    protocol_version: "1" as const, type: "result" as const,
    command: "tms philippines products-export", ok,
  ...
  const zhihuiSkillSnapshot = () => ({
    names: ["zhihui-tms-product-export"],
  ...
  test.each(["确认执行导出", "取消", ""])("preview waits for the card and only confirmation executes: %s", async choice => {
    ...
    const runtime = new TypeScriptAgentRuntime({
      store, tools: new ToolRegistry(), systemPrompt: "test", artifactRoot: "/unused",
      skillSnapshot: zhihuiSkillSnapshot,
      provider: { summarize, turn: async () => { modelCalls++; return messageFixture({ content: [{ type: "text", text: "wrong path" }] }); } },
    ...
    await runtime.start();
    const running = runtime.runTurn(desktopJob(), handle());
    for (let attempt = 0; attempt < 20 && questions.snapshot().length === 0; attempt++) await Bun.sleep(1);
    const pending = questions.snapshot()[0];
    expect(pending?.questions[0]?.options?.map(option => option.label)).toEqual(["确认执行导出", "取消"]);
    expect(pending?.questions[0]?.question).toContain("20260917");
    expect(calls).toEqual(["preview"]);
    expect(modelCalls).toBe(0);
    expect(JSON.stringify(store.messages)).toContain("智汇 TMS 计划");
    questions.submit({ session_id: "s1", request_id: pending!.request_id,
      answers: [{ id: "zhihui_export_confirmation", selected: choice ? [choice] : [] }] });
    const outcome = await running;
    expect(outcome.status).toBe("completed");
    expect(calls).toEqual(choice === "确认执行导出" ? ["preview", "execute"] : ["preview"]);
    expect(modelCalls).toBe(0);
    expect(store.messages.filter(message => message.role === "assistant")).toHaveLength(2);
    await runtime.stop();
  });

  test("stopping while waiting clears the card and never executes", async () => {
    const store = new MemoryStore();
    const questions = new UserQuestionService(() => {});
    const calls: string[] = [];
    ...
    const runtime = new TypeScriptAgentRuntime({
      ...
      skillSnapshot: zhihuiSkillSnapshot,
      provider: { summarize, turn: async () => { throw new Error("unexpected model"); } },
      emitter: { emit: async () => {}, typing: async () => {} },
      zhihuiConfirmation: {
        router: new ZhihuiTmsConfirmationRouter({ execute: async args => {
          const action = args[args.indexOf("--action") + 1]!;
    ...
    await runtime.start();
    const running = runtime.runTurn(desktopJob(), { ...handle(), signal: controller.signal });
    for (let attempt = 0; attempt < 20 && questions.snapshot().length === 0; attempt++) await Bun.sleep(1);
    expect(questions.snapshot()).toHaveLength(1);
    controller.abort();
    const outcome = await running;
    expect(outcome.status).toBe("cancelled");
    expect(questions.snapshot()).toHaveLength(0);
    expect(calls).toEqual(["preview"]);
    expect(store.metrics.at(-1)?.status).toBe("cancelled");
    await runtime.stop();
  });
  ...
  test("failed export still delivers one verified partial merge", async () => {
    ...
    const runtime = new TypeScriptAgentRuntime({
      store, tools: new ToolRegistry(), systemPrompt: "test", artifactRoot,
      skillSnapshot: zhihuiSkillSnapshot,
      provider: { summarize, turn: async () => { throw new Error("unexpected model"); } },
      emitter: { emit: async request => { emitted.push(request); }, typing: async () => {} },
      zhihuiConfirmation: {
        router: new ZhihuiTmsConfirmationRouter({ execute: async args => {
          const action = args[args.indexOf("--action") + 1]!;
  ...
  test("unmatched request still uses model path", async () => {
    const store = new MemoryStore();
    let modelCalls = 0;
    const runtime = new TypeScriptAgentRuntime({
      store, tools: new ToolRegistry(), systemPrompt: "test",
    ...
    await runtime.start();
    const outcome = await runtime.runTurn(desktopJob("你好"), handle());
    expect(outcome.reply).toBe("正常回答");
