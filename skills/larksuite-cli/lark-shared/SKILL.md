...
- path: references/lark-wiki-token-routing.md
  description: references/lark-wiki-token-routing.md
---

# lark-cli 共享规则
## LXE Runtime Rules

- Resolve `SKILL_DIR` as the absolute directory containing the current `SKILL.md`; use it when referencing bundled references, assets, or scripts.
- Before the first Feishu/Lark operation in a task, check `command -v lark-cli`, `lark-cli --version`, and `lark-cli auth status`.
- If `lark-cli` is not installed, not configured, or not authenticated for the required identity, stop and report the missing install/config/authentication step. Do not write credentials, tokens, WorkBuddy connector state, app secrets, or auth files.
- Product/API commands should use machine-readable JSON output when the command supports it. Authentication commands must follow the official flow and use `--json` where documented; help, version, qrcode, and dry-run preview commands are exceptions.
- Run bundled Python helper scripts with `uv run --frozen python "$SKILL_DIR/scripts/<script>.py" ...` from the workspace root. If upstream examples mention `python3 skills/lark-slides/scripts/...`, translate them to the current skill directory before running.
- Put Feishu/Lark downloads, exports, and generated deliverables under workspace-relative `artifacts/lark/`.
- Put transient work directories under workspace-relative `artifacts/lark_work/<unique-task-name>/`, not `/tmp`, so workspace-scoped tools can inspect them.
- For create, update, delete, approval, permission, message-send, recall, or other high-impact operations, run `--dry-run` where supported or show the operation summary and get explicit user confirmation before adding `--yes`.
- Treat exit code `10` with `error.type == "confirmation_required"` as the official confirmation gate: show `error.risk.action` and key parameters to the user, then retry the original argv with `--yes` only after explicit approval.
- When a command path, flag, parameter schema, or current capability is uncertain, verify with the current runtime using `lark-cli <command> --help` or `lark-cli schema ...`; current CLI help/schema overrides stale static documentation.
- CLI runtime is intentionally not pinned to an exact patch version. Keep this vendored skill pack provenance fixed, but adapt execution to the installed `lark-cli` help/schema when safe.
