...
- 仓库中文名和简称必须先归一化为标准代码：马来西亚仓/马来西亚/马来仓/马来 → `MY8801`，菲律宾仓/菲律宾/菲仓 → `PH8805`，泰国仓/泰国/泰仓 → `TH8802`，越南仓/越南/越仓 → `VN8806`。多个仓库按固定顺序去重；中文名不得进入执行参数。
...
| 当前/现在/现有库存、还剩多少货 | `inventory-current-snapshot` |
...
- “当前库存”“现在仓里还有多少货” → `inventory-current-snapshot`。
...
- “MY8801 入库时间”“马来仓入库时间”仍是全局 `inbound-listing-time`，仓库词不进入任务参数，也不因此返回失败。
