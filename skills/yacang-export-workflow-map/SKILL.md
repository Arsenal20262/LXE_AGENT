...
description: 雅仓数据导出的唯一自然语言入口。确定性解析商品创建日期、仓库与标准数据类型，缺省时使用业务默认值；支持完整库存动销、各仓当前库存和入库/上架时间。
...
- “销量/库存动销/库存和销量/动销数据”统一导出完整库存动销 XLSX。报表原样保留 SKU、商品名、仓库、3/7/15/30/60/90 天累计销量、库存、占用、在途、冻结、可用、缺货数量和创建日期；不存在“90 天逐日销量”。
