...
- 用户没提仓库时默认四仓 `MY8801`、`PH8805`、`TH8802`、`VN8806`；明确指定一个或多个时只执行指定仓库。完全没提数据类型时默认四类，但“仓库产品/产品-仓库产品”明确只表示全局入库/上架时间，不能展开成四类。
...
| 用户需求 | 标准 data_type |
| --- | --- |
| 最近一个月/7天/14天/30天/一周/两周销量、月度销售 | `sales-monthly` |
| 近三个月每天/每日/日度/逐日销量、近90天销售趋势 | `sales-90d` |
...
- “最近一个月销量”“最近30天销售”“一周/两周/一个月销量”“近期销售表现” → `sales-monthly`；7/14/30 只是选择固定月度报表，不能生成自定义窗口。
- “近三个月每天销量”“近90天逐日销量”“三个月日销/销售趋势”“每日销量趋势” → `sales-90d`。
