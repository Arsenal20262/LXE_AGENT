...
description: 导出马帮巴西海外仓的原始库存/销量、全部已签收调拨单或默认三个月待签收调拨单 XLSX。用户明确查询巴西海外仓这些数据时使用；不用于 Amazon 店铺补货计算或其他仓库。
...
成功后发送 terminal files 中唯一的 XLSX。不要改名、合并或加工原始文件。
...
- 已签收请求导出所有已签收调拨单：马帮系统-已签收-巴西海外仓-北京时间.xlsx。
- 待签收请求导出 ERP 页面默认三个月范围：马帮系统-3个月待签收-巴西海外仓-北京时间.xlsx。
