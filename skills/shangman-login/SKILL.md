...
5. 根据实际结果说明登录与保存是否成功，并将返回的 `expires_at` 转为用户当地时间报告；有效期采用平台返回值，不固定为 5 分钟或 6 小时。用户纠正后成功时明确说明使用了纠正结果，不宣称 AI 初次识读成功。单独要求登录时登录后结束；作为已授权商品导出的前置步骤时，只有登录成功且已保存后，才返回 `shangman-goods-export` Skill 继续原任务。
