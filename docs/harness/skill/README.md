...
Dashboard → Skills combines managed entries with the Default group, including disabled, invalid and shadowed entries. This is display grouping only: manifest types and permission checks are unchanged. The top-right + menu offers Add skill. Cards open details; managed entries expose an enable switch, Markdown and attached-file previews, Use in conversation, and Move to recycle folder. Details identify the source directory and actual path, not authorship. Official and shared details retain their existing read-only controls.
