...
const makeState = (): ManagedLlmState => withV3Definitions({ revision: 1, default_target: target("cloud-future-flash"),
  models: ["cloud-future-flash", "cloud-future-pro"].map((model, i) => ({ ...target(model), available: true, credential_revision: (i ? "b" : "a").repeat(64) })),
  credentials: ["cloud-future-flash", "cloud-future-pro"].map((model, i) => ({ ...target(model), api_key: "key-" + model, credential_revision: (i ? "b" : "a").repeat(64), fetched_at: 1, invalid_revision: "" })) });
function setup() {
...
test("cloud selection survives restart/default changes; removal selects default and personal selection stays independent", () => {
  ...
  config.saveRuntimePreference("deepseek", "cloud-future-pro", "high", "cloud");
  const restarted = new DesktopConfigStore(root, join(root, "workspace"), safeStorage, { platform: "darwin" });
  expect(restarted.environment().AGENT_LLM_MODEL).toBe("cloud-future-pro");
  expect(restarted.managedLlmTarget()).toEqual(target("cloud-future-pro"));
  ...
  expect(restarted.environment().AGENT_LLM_MODEL).toBe("cloud-future-flash");
  expect(() => restarted.saveRuntimePreference("deepseek", "cloud-future-pro", "high", "cloud")).toThrow();
...
test("v3 sync invalidates known changed keys before fetching and isolates failures while retaining offline cache", async () => {
  ...
  const service = new DesktopCloudService({ dataRoot: root, supported: false, config,
    ...
    fetch: async (input) => {
      ...
      if (url.pathname.endsWith("/identity")) return Response.json({ status: "ok", principal_kind: "managed_device", principal_id: "fixture",
        ...
        managed_llm_v3: { ...state, credentials: undefined } });
      expect(url.searchParams.get("model_schema")).toBe("3");
  ...
  state.models[0]!.definition!.maxTokens = 65536; state.models[0]!.configuration_revision = "e".repeat(64); state.revision++;
  await service.check(); expect(fetched).toHaveLength(2);
  expect(config.managedLlmState().models[0]!.definition!.maxTokens).toBe(65536);
  ...
  (state.models[1]!.definition!.compat as Record<string, unknown>).futureOption = true;
  await service.check(); expect(config.managedLlmState().credentials).toHaveLength(1);
  expect(config.managedLlmState().models[1]!.unavailable_reason).toBe("configuration_unsupported");
  state = { model_schema: 3, revision: 4, default_target: null, models: [], credentials: [] }; await service.check();
