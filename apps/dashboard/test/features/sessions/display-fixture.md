...
从仓库根运行 `bun apps/dashboard/test/features/sessions/display-controller.bench.ts`。
脚本使用合成数据，确认工具行未被窗口裁剪；预热 25 次、测量 60 次，输出中位数和 P95。
只计 `receiveActivity()` 的数据处理，不含 React、网络和初次历史加载。

同机 Bun 1.4.2，固定历史、另一个轮次更新一段文字的结果如下，单位为毫秒：

| 历史调用数 / 轮数 | 更新中位数（前 → 后） | 更新 P95（前 → 后） |
| --- | --- | --- |
| 500 / 1 | 3.059 → 0.128 | 3.733 → 0.411 |
| 1,000 / 1 | 9.183 → 0.223 | 10.002 → 0.554 |
| 2,000 / 1 | 29.942 → 0.560 | 32.147 → 1.029 |
| 500 / 20 | 1.406 → 0.135 | 1.924 → 0.418 |
| 1,000 / 20 | 2.771 → 0.244 | 3.246 → 0.615 |
| 2,000 / 20 | 6.057 → 0.476 | 6.818 → 0.886 |

实时轮次本身有 2,000 次工具调用时，仍按整轮重算，中位数为 0.975 → 0.953 ms，
P95 为 1.464 → 1.653 ms；这部分没有承诺逐行增量收益。数字受负载影响，不作 CI 耗时门槛。
新增行为测试检查历史内容不会因文字增量被重读、历史行及尾注引用复用、缓存覆盖不发生原地修改、
窗口替换时缓存失效，以及工具 ID 读取次数随输入线性增长。
